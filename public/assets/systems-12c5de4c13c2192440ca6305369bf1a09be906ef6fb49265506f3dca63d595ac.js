...
app.controller("SystemsController", ["$scope", "$http", "$mdToast", "$mdDialog", "$mdEditDialog", "$mdMedia", function($scope, $http, $mdToast, $mdDialog, $mdEditDialog, $mdMedia) {
        ...
        if ($scope.editing) {
            $mdEditDialog.small({
                ...
                save: function(system) {
                    $http.patch($scope.systems_url + "/" + system.id, buildRequest(system).then($scope.ajaxSuccess, $scope.ajaxFailure));
                },
                targetEvent: event,
                validators: {
                    "md-maxlength": 15
    ...
    $scope.editComponents = function (event, systemToUpdate) {
        $mdDialog.show({
            ...
            var index = indexOfItemWithID(systemToUpdate.id, $scope.systems);
            if (index != -1) {
                $scope.system[index].exam_techniques = exam_techniques;
            }
            $http.patch($scope.systems_url + "/" + $scope.system[index].id, buildRequest($scope.system[index]).then($scope.ajaxSuccess, $scope.ajaxFailure));
        });
    };
