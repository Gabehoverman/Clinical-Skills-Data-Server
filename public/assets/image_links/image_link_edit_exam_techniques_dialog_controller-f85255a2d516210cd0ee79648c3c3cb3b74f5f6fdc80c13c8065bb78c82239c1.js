function EditExamTechniquesDialogController($scope, $mdDialog, imageLink, allExamTechniques, editing) {
    ...
    $scope.filterExamTechniques = function () {
        for (var i = 0; i < $scope.allExamTechniques.length; i++) {
            ...
            if (($scope.isInitialFilter) ? indexOfItemWithID(examTechnique.id, $scope.imageLink.exam_techniques) == -1 : indexOfItemWithID(examTechnique.id, $scope.usedExamTechniques) == -1) {
                if (indexOfItemWithID(examTechnique.id, $scope.unusedExamTechniques) == -1) {
                    if (!examTechnique.imageLink) {
    ...
    $scope.remove = function(examTechniqueToRemove) {
        ...
        if (index != -1) {
            ...
            if (allIndex != -1) {
                $scope.allExamTechniques[allIndex].imageLink = null;
            }
    ...
    $scope.addExamTechnique = function() {
        if($scope.examTechniqueToAdd) {
            ...
            if (allIndex != -1) {
                $scope.allExamTechniques[allIndex].imageLink = { "name": $scope.imageLink.name };
            }
