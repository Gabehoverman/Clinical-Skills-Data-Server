app.controller("SubsystemsController", ["$scope", "$http", "$mdToast", "$mdDialog", "$mdEditDialog", "$mdMedia", function($scope, $http, $mdToast, $mdDialog, $mdEditDialog, $mdMedia) {
	...
	$scope.save = function() {
		if ($scope.changes.updates.length !== 0) {
			for (var i = 0; i < $scope.changes.updates.length; i++) {
				$http.patch(subsystems_base_url + "/" + $scope.changes.updates[i].id, buildRequest($scope.changes.updates[i])).then($scope.ajaxSuccess, $scope.ajaxFailure);
		...
		if ($scope.changes.deletes.length !== 0) {
			for (var j = 0; j < $scope.changes.deletes.length; j++) {
				$http({
					method: 'DELETE',
					url: subsystems_base_url + "/" + $scope.changes.deletes[j].id,
					dataType: 'json',
					headers: {
	...
	$scope.delete = function(subsystemToDelete) {
		$scope.changes.deletes.push(subsystemToDelete);
	...
	$scope.newSubsystem = function(event) {
		$mdDialog.show({
	      ...
	      fullscreen: $mdMedia('xs') || $mdMedia('sm')
    		...
    		$http.post(subsystems_base_url, buildRequest(newSubsystem)).then($scope.ajaxSuccess, $scope.ajaxFailure);
		});
	};
