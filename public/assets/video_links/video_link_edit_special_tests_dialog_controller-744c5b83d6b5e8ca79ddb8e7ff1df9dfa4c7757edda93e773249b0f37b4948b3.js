function EditSpecialTestsController($scope, $mdDialog, videoLink, allSpecialTests, editing) {
    ...
    $scope.filterSpecialTests = function () {
        for (var i = 0; i < $scope.allSpecialTests.length; i++) {
            ...
            if (($scope.isInitialFilter) ? indexOfItemWithID(specialTest.id, $scope.videoLink.special_tests) == -1 : indexOfItemWithID(specialTest.id, $scope.usedSpecialTests) == -1) {
                if (indexOfItemWithID(specialTest.id, $scope.unusedSpecialTests) == -1) {
                    if (!specialTest.videoLink) {
    ...
    $scope.remove = function(specialTestToRemove) {
        ...
        if (index != -1) {
            ...
            if (allIndex != -1) {
                $scope.allSpecialTests[allIndex].videoLink = null;
            }
    ...
    $scope.addSpecialTest = function () {
        if($scope.specialTestToAdd) {
            ...
            if (allIndex != -1) {
                $scope.allSpecialTests[allIndex].videoLink = { "name": $scope.videoLink.name };
            }
