...
heent                    = System.create(name: 'Head, Ear, Eyes, Nose, Throat', details: '')
...
cardiovascular_component = Component.create(system: cardiovascular, name: 'Cardiovascular', inspection: '', notes: '')
...
posterior_drawer_test           = SpecialTest.create(component: knee, name: 'Posterior Drawer Test', positive_sign: 'Abnormal backward mobility of 2 cm or more', indication: 'Posterior cruciate ligament tear', notes: 'Tests for posterior cruciate ligament tear.  With the patient supine, have the patient flex the knee to 45-90°.  Stabilize the leg by gently sitting on the patient’s foot.  Then, grasp the leg just below the knee with both hands and push the tibia posteriorly.  Abnormal backward mobility of 2 cm or more is a positive test')
lachmans_test                   = SpecialTest.create(component: knee, name: 'Lachman\'s Test', positive_sign: 'Increased laxity in the anterior cruciate ligament', indication: 'Anterior cruciate ligament tear', notes: 'Tests for anterior cruciate ligament tear.  Have the patient lie supine with the extremity to be tested next to the examiner. The knee is positioned between 20 and 30° flexion.  Stabilize the distal lateral femur with one hand and grasp the medial proximal tibia with the other hand with the fingers posterior and the thumbs anterior and medial.  Apply a pressure through the fingers and the palm of the hand that translates the tibia anteriorly.  The test is positive when there is increased laxity in the anterior cruciate ligament.')
...
homans_sign_test              = SpecialTest.create(component: ankle_foot, name: 'Homan\'s Sign Test', positive_sign: 'Pain in the calf with dorsiflexion of the foot', indication: 'DVT', notes: 'Tests for DVT in the calf.  Dorsiflex the foot. The test is positive when dorsiflexion elicits pain in the calf area.  (Sensitivity:  10-54%; Specificity:  38-89%).')
ankle_drawer_sign_test        = SpecialTest.create(component: ankle_foot, name: 'Ankle Drawer Sign Test', positive_sign: 'Obvious extreme forward displacement of the entire foot on the tibia', indication: 'Anterior talofibular ligament injury', notes: 'Tests for disruption of anterior talofibular ligament.  The following 3 things are necessary to detect a positive drawer sign; First:  the anterior and posterior muscles which actuate the ankle must be relaxed.  This achieved by having the patient in either the sitting or supine position with the knee flexed to 90°. Second:  the ankle must be positioned at 10 to 20°plantarflexion.  Third:  for right ankle testing, place left hand over anterior distal tibia proximal to ankle joint; place the right hand on the medial surface curving the fingers over the posterior calcaneus.  Pull forward on the heel while pushing posteriorly on the tibia.  Repeat with the left side.  A positive test is when there is obvious extreme forward displacement of the entire foot on the tibia (when compared to the contralateral side).')

capillary_refill_test = SpecialTest.create(component: cardiovascular_component, name: 'Capillary Refill', positive_sign: 'Capillary refill time of longer than 2 seconds', indication: 'Dehydration or peripheral perfusion issue', notes: '')
pitting_edema_test    = SpecialTest.create(component: cardiovascular_component, name: 'Pitting Edema', positive_sign: 'Depression that does not rapidly refill and resume its original contour', indication: 'Edema, increased venous hydrostatic pressure, or heart failure', notes: 'If grading of pitting edema is not recalled a quick review of how to might not be a bad idea')
moses_sign_test       = SpecialTest.create(component: cardiovascular_component, name: 'Bancroft/Moses\' Sign Test', positive_sign: 'Pain in the calf with anteroposterior compression', indication: 'DVT', notes: '')
cardiovascular_component.special_tests << homans_sign_test

chest_excursion_test        = SpecialTest.create(component: respiratory_component, name: 'Chest Excursion', positive_sign: 'Asymmetric movement of hands', indication: 'Problem with one or both lungs (usually decreased in a barrel chest)', notes: '')
crepitus_test               = SpecialTest.create(component: respiratory_component, name: 'Crepitus Test', positive_sign: 'Crackly sensation or sound', indication: 'Presence of air in the subcutaneous tissue', notes: '')
pleural_friction_rub_test   = SpecialTest.create(component: respiratory_component, name: 'Pleural Friction Rub Test', positive_sign: 'Grating vibration felt upon palpation', indication: 'Inflammation of the pleural space', notes: '')
tactile_fremitus_test       = SpecialTest.create(component: respiratory_component, name: 'Tactile Fremitus Test', positive_sign: 'Increased or decreased sound transmission', indication: 'Increased sound transmission from fluid or a solid mass (i.e. tumor) or decreased sound transmission from excessive air (i.e. emphysema or bronchial obstruction)', notes: '')
bronchophony_test           = SpecialTest.create(component: respiratory_component, name: 'Bronchophony Test', positive_sign: 'Increased sound when the patient is asked repeat a phrase (“99, 99”)', indication: 'Consolidation', notes: '')
whispered_pectoriloquy_test = SpecialTest.create(component: respiratory_component, name: 'Whispered Pectoriloquy Test', positive_sign: 'Increased sound that is louder and clearer when the patient is asked to whisper “1-2-3”', indication: 'Consolidation', notes: '')
egophony_test               = SpecialTest.create(component: respiratory_component, name: 'Egophony Test', positive_sign: 'When patient is asked to say “E-E-E-E” it is heard as “A-A-A-A” (it is imperative that you ask the patient to repeat this as an individual sound and not as a continual one)', indication: 'Consolidation', notes: '')

brudzinski_sign = SpecialTest.create(component: neurological_component, name: 'Brudzinski Sign Test', positive_sign: 'Nuchal pain with flexion of the hips and lower extremities', indication: 'Meningeal irritation (i.e. meningitis or intracranial hemorrhage)', notes: '')
kernig_sign     = SpecialTest.create(component: neurological_component, name: 'Kernig Sign Test', positive_sign: 'Low back pain', indication: 'meningeal irritation (i.e. meningitis or intracranial hemorrhage)', notes: '')

murphys_sign               = SpecialTest.create(component: abdomen_component, name: 'Murphy\'s Sign', positive_sign: 'Pain and sudden halting of respiration', indication: 'Gallbladder inflammation', notes: '')
...
rebound_tenderness_test    = SpecialTest.create(component: abdomen_component, name: 'Rebound Tenderness Test', positive_sign: 'Stabbing pain occurs with rapid withdrawal of hands', indication: 'Peritoneal inflammation', notes: 'Blumberg\'s Sign')
...
obturator_muscle_test      = SpecialTest.create(component: abdomen_component, name: 'Obturator Muscle Test', positive_sign: 'Pain upon flexion at the hip and knee with internal and external rotation of the right leg', indication: 'Appendicitis or pelvic abscess', notes: '')
...
VideoLink.create(special_tests: [homans_sign_test], title: 'Homan\'s Sign Test', link: 'https://www.youtube.com/watch?v=5-LCDB_PPYM')
...
VideoLink.create(special_tests: [moses_sign_test], title: 'Moses\' Sign', link: 'https://www.youtube.com/watch?v=b1oaLDj1u4Y')
VideoLink.create(special_tests: [chest_excursion_test], title: 'Chest Excursion', link: 'https://www.youtube.com/watch?v=8IjMjFSRmII')
VideoLink.create(special_tests: [tactile_fremitus_test], title: 'Tactile Fremitus', link: 'https://www.youtube.com/watch?v=NoJeTljJ-PQ')
VideoLink.create(special_tests: [bronchophony_test], title: 'Bronchophony', link: 'https://www.youtube.com/watch?v=NlfUy3yyAEs')
VideoLink.create(special_tests: [whispered_pectoriloquy_test], title: 'Whispered Pectoriloquy', link: 'https://www.youtube.com/watch?v=V761MusII1I')
VideoLink.create(special_tests: [egophony_test], title: 'Egophony', link: 'https://www.youtube.com/watch?v=VthSqLviduk')
VideoLink.create(special_tests: [brudzinski_sign], title: 'Brudzinski Sign', link: 'https://www.youtube.com/watch?v=Pxo02mAgIvg')
VideoLink.create(special_tests: [kernig_sign], title: 'Kernig Sign', link: 'https://www.youtube.com/watch?v=EV1HOuxJUjQ')
VideoLink.create(special_tests: [murphys_sign], title: 'Murphy\'s Sign', link: 'https://www.youtube.com/watch?v=Uk0zQUZphlI')
